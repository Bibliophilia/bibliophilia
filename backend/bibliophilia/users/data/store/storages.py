...
class GroupStorageImpl(GroupStorage):
    ...
    def create(self, group: GroupCreate) -> Optional[Group]:
        with Session(self.engine) as session:
            ...
            #    user = session.query(User).filter(User.email == group_user).first()
                ...
                #session.add(user)
                users.append(user)
            #users = session.exec(select(User).where(or_(*(User.email == user_email for user_email in group.users)))).all()
