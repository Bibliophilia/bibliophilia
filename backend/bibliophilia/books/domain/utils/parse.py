...
def parse_facets(query: str) -> (str, dict):
    ...
    for facet in Facet:
        ...
        matches = re.findall(fr'-?{fc.value}:{fc.mather()}', final_query)
    ...
    must = ([{"terms": {arr+"s.value.keyword": rules_map["must"]["terms"][arr]}} for arr in rules_map["must"]["terms"]]
            + [{"range": {dictionary+"s.value.keyword": rules_map["must"]["range"][dictionary]}} for dictionary in rules_map["must"]["range"]])
    must_not = ([{"terms": {arr+"s.value.keyword": rules_map["must_not"]["terms"][arr]}} for arr in rules_map["must_not"]["terms"]]
                + [{"range": {dictionary+"s.value.keyword": rules_map["must_not"]["range"][dictionary]}} for dictionary in rules_map["must_not"]["range"]])
